...
export function printWarning(message: string) {
  ...
  log.enableColor();
  log.warn('TR_CDK_CONSTRUCTS_WARNING: ', message);
}
...
export function generateFileHash(filePath: string) {
  ...
  const normalizedData = fileBuffer.toString().replace(/\r\n/g, '\n');
  const hashSum = crypto.createHash('sha256');
  hashSum.update(normalizedData, 'utf-8');
...
export function toRegionShortcut(region: string): string | undefined {
  const m = region.match(/^([a-z]+)-(north|south|west|east)(west|east)?-([0-9]+)$/);
  if (m === null) { // Probably using the default region placeholder, ignore in this case.
    return undefined;
  }
  const country = m[1];
  let location = m[2].charAt(0);
  if (m[3] !== undefined) {
    location += m[3].charAt(0);
  }
  const num = m[4];

  return `${country}${location}${num}`;
}
