...
export const handler: CloudFrontRequestHandler = async (event) => {
  ...
  const accessTokenEndpoint = CONFIG.accessTokenEndpoint;
  ...
  try {
    ...
    const requestConfig: Parameters<
      typeof common.httpPostToIDPWithRetry
    >[2] = {
      headers: {
    ...
    CONFIG.logger.debug('HTTP POST to IDP token endpoint:\n', {
    ...
    CONFIG.logger.debug('Response from IDP token endpoint:\n', {
      status,
      headers,
      idToken,
      accessToken,
...
function validateQueryStringAndCookies(props: {
  ...
  // Check if IDP threw an Error. IDP puts the error in the query string
  const {
    code,
    state,
    error: cognitoError,
