...
export const handler: CloudFrontRequestHandler = async (event) => {
  ...
  try {
    ...
    CONFIG.logger.debug('Passed validateRefreshRequest');
    ...
    const resAccessToken = await common
    ...
    CONFIG.logger.debug(`resAccessToken: ${JSON.stringify(resAccessToken)}`);
    newAccessToken = resAccessToken.data.access_token as string;

    const encodedSecretIdToken = Buffer.from(newAccessToken);
    headers.Authorization = `Basic ${encodedSecretIdToken}`;
    ...
    const response = {
      ...
      headers: {
        'location': [
          {
            key: 'location',
            value: `https://${domainName}${common.ensureValidRedirectPath(requestedUri)}`,
          },
        ],
    ...
    CONFIG.logger.debug('Passed Temporary Redirect');
    CONFIG.logger.debug('Returning response:\n', response);
    return response;
  } catch (err) {
    CONFIG.logger.debug('err: ', err);
    const response = {
